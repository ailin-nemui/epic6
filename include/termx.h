...
#define term_reverse_on()		term_emit("rev")
#define term_reverse_off()		term_emit("rmrev")
#define term_italics_on()		term_emit("sitm")
#define term_italics_off()		term_emit("ritm")
	void		term_set_foreground	(int);
	void		term_set_background	(int);
	void    	term_set_rgb_foreground (int red, int green, int blue);
	void    	term_set_rgb_background (int red, int green, int blue);

#define term_cr()			term_emit("cr")
#define term_newline()			term_emit("nel")
#define term_cursor_right()		term_emit("cuf1")
#define term_all_off()          	term_emit("sgr0")
	...
	/* Output a terminfo attribute by its name */
	void    	term_emit 		(const char *attribute);

	/* Output a hardcoded string */
	void		term_raw_bytes		(const char *bytes);

	/* Output just one character (but it gets line buffered) */
	/* Make sure to call term_flush() when you're done! */
	int     	term_output_char 	(int c);

#define term_flush()		term_output_char(0)
