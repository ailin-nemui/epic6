 ...
 * escape sequences.
 *
 * "Why do we use (FILE *)s instead of fd's for stdout?" I keep asking myself.
 * For line buffering, I have to keep reminding myself.
 * But that doesn't mean that in $CURRENT_YEAR it makes a difference
 */
 ...
 * IMPORTANT!
 *
 * These values here get passed to tiparm().  
 * tiparm() expects all integer values to be (int) and nothing else.
 * Never change these without considering that!
 */
...
#define term_reverse_on()		tputs_x(current_term->TI_rev)
#define term_reverse_off()		tputs_x("\033[27m")
#define term_italics_on()		tputs_x(current_term->TI_sitm)
#define term_italics_off()		tputs_x(current_term->TI_ritm)
...
#define term_cursor_right()		tputs_x(current_term->TI_cuf1)
#define term_all_off()          	tputs_x(current_term->TI_sgr0)

	/* Housekeeping functions */
	int		term_init 		(void);	   /* startup */
	int		term_resize 		(void);	   /* periodic function */
	void		term_cont 		(int);	   /* signal handler */
	void		term_reset		(void);	   /* shutdown */
	void		set_automargin_override	(void *);

	/* Metadata functions */
	const char *	get_term_capability	(const char *, int, int);
	void		get_term_geometry	(int *, int *);
	...
	void		term_sgr		(int, int, int, int, int, int, int, int, int);
	void 		term_beep 		(void);
	/* Don't forget tputs_x() and the term_* stuff above */
